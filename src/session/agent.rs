...
///
/// Minimal preamble — the BMAD skill handles the workflow. The daemon only
/// injects the English override and the completion sentinel.
pub(crate) fn build_review_preamble() -> String {
    r#"OVERRIDE: communication_language = English

When you have completed the entire workflow, output <<BMAD_JOB_DONE>> on its own line in a separate final response to signal you are finished."#
        .to_string()
}
