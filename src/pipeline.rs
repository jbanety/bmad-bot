...
///
/// Implemented by [`SessionRunner`] for production use. Integration tests
/// substitute a `MockDevRunner` that returns canned [`SessionOutcome`] values.
#[async_trait]
...
///
/// Implemented by [`ReviewRunner`] for production use. Integration tests
/// substitute a `MockCodeReviewer` that returns canned [`ReviewOutcome`] values.
#[async_trait]
...
pub struct StoryPipeline {
    ...
    /// Code review runner (trait object for testability).
...
impl StoryPipeline {
    ...
    pub fn new(
        ...
        // Second SessionRunner instance for WAL recovery — cheap to construct (no state, no network).
        // Must be created before `shutdown`/`mcp_manager` are moved into ReviewRunner.
    ...
    ///
    /// Accepts trait objects for all pluggable components. `session_runner_for_recovery`
    /// is set to `None` — `recover_and_process()` returns `None` (safe no-op).
