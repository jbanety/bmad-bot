...
- Pre-existing test failure: `session::cleanup::tests::test_unblock_dependents_no_partial_key_match` (asserts len==1 but gets 2). Not introduced by this story — `src/session/cleanup.rs` was NOT modified.

### Completion Notes List
- **Task 0**: Created `src/lib.rs` with `pub mod` for all modules (config, git_provider, notifier, pipeline, review, session, supervisor, tools, watcher, auth, llm_context, llm_logging). Removed mod declarations from `main.rs` (kept `mod cli;`). Updated all `crate::` references in `src/cli/` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage}` re-export in `session/mod.rs`. Build and 755 unit tests pass.
- **Task 1**: Created `tests/integration.rs` entry point with `#[path]` attributes for edition 2024 compatibility. Created `tests/integration/helpers/{mod.rs, mocks.rs, fixtures.rs}` and test files.
- **Task 2**: `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), `GitProvider` trait impl, call tracking via `Arc<Mutex<Vec<GitProviderCall>>>`, `Send + Sync` verified.
- **Task 3**: `MockNotifier` with `Notifier` trait impl, captures all calls, provides `calls()`, `story_calls()`, `summary_calls()` accessors, `Send + Sync` verified.
- **Task 4**: `MockSessionRunner` standalone struct with configurable `SessionOutcome`, `run()` and `check_and_recover_wal()` methods, call tracking, `Send + Sync` verified.
- **Task 5**: `MockReviewRunner` standalone struct with configurable `ReviewOutcome`, `run()` method, call tracking, `Send + Sync` verified.
- **Task 6**: All 6 fixture builders implemented: `make_test_config`, `make_test_secrets`, `make_test_story`, `write_sprint_status`, `write_wal_file`, `create_test_repo`.
- **Task 7**: 36 self-verification tests covering all mocks and fixtures. All pass. Tests use `tempfile::tempdir()` for isolation. Tests validate parseable YAML output, valid git repos, Send+Sync bounds.
- **Decision**: Added `git2 = "0.20"` to `[dev-dependencies]` since integration tests use `git2::Repository` directly and dev-dependencies are separate from main dependencies.
- **Decision**: Used `#[path = "..."]` attributes in `tests/integration.rs` for Rust edition 2024 module resolution compatibility.
- **Decision**: Included `auth`, `llm_context`, `llm_logging` as `pub mod` in `lib.rs` (in addition to story-specified modules) because other library modules reference them via `crate::`. Without these, compilation fails.

### File List
- `src/lib.rs` — NEW: library crate root exposing all modules for integration tests
- `src/main.rs` — MODIFIED: removed mod declarations (kept mod cli), now binary-only
- `src/session/mod.rs` — MODIFIED: added `pub use state::{SessionState, ChatMessage}` re-export
- `src/cli/mod.rs` — MODIFIED: changed `crate::` imports to `bmad_bot::` for library modules
...
- `tests/integration/test_mocks.rs` — NEW: 22 self-verification tests for mock implementations
- `tests/integration/test_fixtures.rs` — NEW: 14 self-verification tests for fixture builders
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — MODIFIED: story 7-1 status → in-progress