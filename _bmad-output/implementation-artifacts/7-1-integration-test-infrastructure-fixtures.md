...
- Task 0: Created `src/lib.rs` with `pub mod` for all 11 non-CLI modules (auth, config, git_provider, llm, notifier, pipeline, review, session, supervisor, tools, watcher). Updated `src/main.rs` to only declare `mod cli;`. Updated all `crate::` references in `src/cli/mod.rs` and `src/cli/state.rs` to `bmad_bot::` since CLI is now in binary crate. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 849 lib + 104 bin unit tests pass.
- Task 1: Created `tests/integration.rs` entry point with `#[path]` attributes (needed because test crate roots don't auto-resolve companion directories). Created `tests/integration/helpers/mod.rs`, `mocks.rs`, `fixtures.rs`.
- Task 2: Implemented `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), `Arc<Mutex<...>>` for return values, `GitProviderCall` enum for call tracking, and `mock_git_error()` helper to clone non-Clone error variants.
- Task 3: Implemented `MockNotifier` with `NotifierCall` enum, `calls()`/`story_calls()`/`summary_calls()` accessors, configurable error returns.
- Task 4: Implemented `MockSessionRunner` with closure-based outcome factory (`with_outcome`), `SessionRunCall` tracking, `check_and_recover_wal()` returning None.
- Task 5: Implemented `MockReviewRunner` with same pattern as session runner.
- Task 6: Implemented all 6 fixture builders: `make_test_config`, `make_test_secrets`, `make_test_story`, `write_sprint_status`, `write_wal_file`, `create_test_repo`. Used real `SprintStatusFile::load()` in tests to verify YAML compatibility.
- Task 7: 34 integration tests covering all mocks, fixtures, and Send+Sync bounds. All pass.

### File List
- `src/lib.rs` — NEW: Library crate root exposing all non-CLI modules
- `src/main.rs` — MODIFIED: Removed all `mod` declarations except `cli`, binary-only
- `src/session/mod.rs` — MODIFIED: Added `pub use state::{SessionState, ChatMessage};`
- `src/cli/mod.rs` — MODIFIED: Changed `crate::` references to `bmad_bot::`
- `src/cli/state.rs` — MODIFIED: Changed `crate::` references to `bmad_bot::`
...
- `tests/integration/test_mocks.rs` — NEW: 23 mock verification tests
- `tests/integration/test_fixtures.rs` — NEW: 11 fixture verification tests
