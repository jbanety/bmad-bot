...
- **Task 0**: Created `src/lib.rs` exposing all non-CLI modules (`auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Updated `src/main.rs` to keep only `mod cli;` — all `crate::` references in CLI modules changed to `bmad_bot::`. Changed `pub(crate) mod state;` → `pub mod state;` in `src/session/mod.rs`. All 830 existing unit tests pass.
- **Task 1**: Created `tests/integration.rs` entry point with `#[path]` attributes pointing into `tests/integration/` directory. Created `helpers/mod.rs`, `helpers/mocks.rs`, `helpers/fixtures.rs`, `test_mocks.rs`, `test_fixtures.rs`.
- **Task 2**: `MockGitProvider` with `Arc<Mutex<...>>` fields, builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), full `GitProvider` trait impl, call-tracking Vec with `GitProviderCall` enum. Includes `clone_git_provider_error` helper covering all 11 `GitProviderError` variants.
- **Task 3**: `MockNotifier` with `Arc<Mutex<Vec<NotifierCall>>>`, full `Notifier` trait impl, `calls()`, `story_calls()`, `summary_calls()` accessors.
- **Task 4**: `MockSessionRunner` with configurable outcome factory `Box<dyn Fn(&StoryInfo) -> SessionOutcome + Send>`, `run()` + `check_and_recover_wal()` methods, call tracking.
- **Task 5**: `MockReviewRunner` with configurable outcome factory, `run()` method, call tracking.
...
- **Task 7**: 36 self-verification tests covering all mocks, all fixtures, Send+Sync bounds, YAML roundtrips, git repo initialization.
- **Decision**: Used `#[path = "integration/..."]` attributes in `tests/integration.rs` because Rust edition 2024 resolves submodules relative to parent directory (`tests/`), not relative to the file itself. This is the standard workaround.
- **Decision**: `BotSecrets.github_copilot_oauth_token` is the actual field name (not `github_models_api_key` as noted in story). Used actual field name.

### File List
- `src/lib.rs` — NEW: library crate root exposing all non-CLI modules
- `src/main.rs` — MODIFIED: removed mod declarations, kept `mod cli;` only
- `src/session/mod.rs` — MODIFIED: `pub(crate) mod state;` → `pub mod state;`
- `src/cli/mod.rs` — MODIFIED: `crate::` → `bmad_bot::` for 13 references
- `src/cli/state.rs` — MODIFIED: `crate::config` → `bmad_bot::config` (2 refs)
- `src/cli/git_detect.rs` — MODIFIED: doc comment `crate::` → `bmad_bot::`
...
- `tests/integration/test_mocks.rs` — NEW: 27 mock verification tests + 4 Send+Sync tests
- `tests/integration/test_fixtures.rs` — NEW: 17 fixture verification tests