...
None needed — all tasks completed cleanly.

### Completion Notes List
- Task 0: Created `src/lib.rs` exposing all non-CLI modules (`auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Updated `src/main.rs` to keep only `mod cli;`. Updated all `crate::` references in `cli/mod.rs`, `cli/state.rs` to use `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 830 lib + 104 bin unit tests pass.
- Task 1: Created `tests/integration.rs` entry point with `#[path]` attributes (required by Rust 2024 edition), `tests/integration/helpers/mod.rs`, `tests/integration/helpers/mocks.rs`, `tests/integration/helpers/fixtures.rs`.
- Task 2: Implemented `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), `Arc<Mutex<...>>` interior mutability, call tracking via `GitProviderCall` enum, and `#[async_trait] impl GitProvider`.
- Task 3: Implemented `MockNotifier` with `Arc<Mutex<Vec<NotifierCall>>>` capturing all calls, `calls()`, `story_calls()`, `summary_calls()` accessors.
- Task 4: Implemented `MockSessionRunner` with `new_completed()` and `with_outcome(F)` factory constructors, `run()` + `check_and_recover_wal()` methods, call tracking.
- Task 5: Implemented `MockReviewRunner` with `new_completed()` and `with_outcome(F)`, `run()` method, call tracking.
- Task 6: Implemented all fixture builders: `make_test_config(dir)`, `make_test_secrets()`, `make_test_story(key, label, deps)`, `write_sprint_status(dir, entries)`, `write_wal_file(dir, state)`, `create_test_repo(dir)`, `make_test_session_state(key)` helper.
- Task 7: 36 self-verification integration tests covering all mocks (return values, error paths, call tracking, Send+Sync bounds) and all fixtures (config validation, key parsing, YAML roundtrips, git repo init).
- Decision: Added `auth` and `llm` modules to `lib.rs` beyond what the story template specified, because `cli` module references `crate::auth` and `pipeline.rs` references `crate::llm` — required for compilation.
- Decision: Used `#[path = "..."]` attributes in `tests/integration.rs` because Rust 2024 edition changed module resolution for integration test files.

### File List
- `src/lib.rs` — NEW: Library crate entry point exposing all non-CLI modules
- `src/main.rs` — MODIFIED: Removed all `mod` declarations except `mod cli;`
- `src/session/mod.rs` — MODIFIED: Added `pub use state::{SessionState, ChatMessage};` re-export
- `src/cli/mod.rs` — MODIFIED: Changed `crate::` references to `bmad_bot::` for auth, config, session, watcher, pipeline
...
- `tests/integration/test_fixtures.rs` — NEW: 16 self-verification tests for fixtures