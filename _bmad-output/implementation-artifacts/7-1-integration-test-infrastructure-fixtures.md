...
Pre-existing test failure: `session::cleanup::tests::test_unblock_dependents_no_partial_key_match` — regex `\b` word boundary matches before hyphens, causing false positive match on `7-1-infra-extended` when searching for `7-1-infra`. Not introduced by this story. 810 lib tests pass, 1 pre-existing failure.

### Completion Notes List
- Task 0: Created `src/lib.rs` exposing all modules except `cli` (11 public modules: auth, config, git_provider, llm, notifier, pipeline, review, session, supervisor, tools, watcher). Updated `src/main.rs` to keep only `mod cli;`. Updated all `crate::` references in CLI module to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. Story Dev Notes listed only 9 modules but `auth` and `llm` were required by other modules (session, review, pipeline, supervisor all use `crate::llm`; `llm/agent_factory.rs` uses `crate::auth`).
- Task 0.5: `cargo build` succeeds with no errors, `cargo test --lib` passes 810/811 (1 pre-existing failure in cleanup.rs, unrelated).
- Task 1: Created integration test directory structure with `#[path]` attributes in `tests/integration.rs` due to edition 2024 module resolution not supporting implicit `tests/integration/` submodule directory for test crate roots.
- Tasks 2-5: Implemented all four mocks with builder-pattern configuration and Arc<Mutex<...>> call tracking. Used manual clone functions instead of deriving Clone on error types (GitProviderError, NotifierError, SessionOutcome, ReviewOutcome don't derive Clone).
- Task 6: Implemented all 6 fixture builders. `write_sprint_status` writes all entry types (epics, stories, retrospectives) under `development_status:`. `create_test_repo` uses `drop(tree)` before returning repo to satisfy borrow checker.
- Task 7: 35 integration tests covering all mocks, fixtures, and Send+Sync bounds. All pass.

### File List
- `src/lib.rs` (NEW) — Library crate root exposing all modules for integration tests
- `src/main.rs` (MODIFIED) — Removed module declarations, kept only `mod cli;`
...
- `src/cli/git_detect.rs` (MODIFIED) — Updated doc comment reference
