...
- **Task 0:** Created `src/lib.rs` exposing all non-CLI modules (`auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Updated `src/main.rs` to keep only `mod cli;`. Updated all `crate::` references in `src/cli/mod.rs` and `src/cli/state.rs` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 830 lib + 104 bin unit tests pass.
- **Task 1:** Created `tests/integration.rs` entry point with `#[path]` attributes (edition 2024 requires explicit paths for test binary submodules). Created `tests/integration/helpers/mod.rs`, `mocks.rs`, `fixtures.rs`.
- **Task 2:** Implemented `MockGitProvider` with `Arc<Mutex<...>>` for configurable return values, full `GitProvider` trait impl, builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), and call tracking via `GitProviderCall` enum.
- **Task 3:** Implemented `MockNotifier` with `Arc<Mutex<Vec<NotifierCall>>>` capturing all notifications. Provides `calls()`, `story_calls()`, `summary_calls()` accessors.
- **Task 4:** Implemented `MockSessionRunner` as standalone struct with configurable `SessionOutcome` via closure factory. Tracks calls via `SessionRunnerCall`. `check_and_recover_wal()` returns `None`.
- **Task 5:** Implemented `MockReviewRunner` as standalone struct with configurable `ReviewOutcome` via closure factory. Tracks calls via `ReviewRunnerCall`.
- **Task 6:** Implemented all 6 fixture builders: `make_test_config()`, `make_test_secrets()`, `make_test_story()`, `write_sprint_status()`, `write_wal_file()`, `create_test_repo()`. All produce valid, parseable data. `BotSecrets` field name `github_copilot_oauth_token` used (matches actual codebase, not story's original `github_models_api_key`).
- **Task 7:** 34 integration tests covering all mocks and fixtures. All pass. `Send + Sync` bounds verified for all 4 mock types.
...
- `src/cli/mod.rs` — MODIFIED: `crate::` → `bmad_bot::` for non-CLI module references
- `src/cli/state.rs` — MODIFIED: `crate::` → `bmad_bot::` for config discovery reference
...
- `tests/integration/test_fixtures.rs` — NEW: 13 fixture self-verification tests
