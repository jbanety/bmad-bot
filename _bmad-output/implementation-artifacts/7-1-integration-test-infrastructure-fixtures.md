...
- **Task 0:** Expanded `src/lib.rs` from 2 modules (config, mcp) to 12 modules (auth, config, git_provider, llm, mcp, notifier, pipeline, review, session, supervisor, tools, watcher). Added `pub use state::{SessionState, ChatMessage}` re-export in `src/session/mod.rs`. Kept `mod X;` declarations in `main.rs` to preserve existing `crate::` references in CLI module (dual-crate compilation pattern already established in project). All 990 unit tests pass.
- **Task 0.2 deviation:** Did NOT remove `mod X;` from `main.rs` because CLI module has 19 `crate::` path references. Removing them would require changing all to `bmad_bot::` — out of scope. The dual-compilation pattern was already established (`config` and `mcp` had both `mod` in `main.rs` and `pub mod` in `lib.rs`).
...
- **Task 2:** `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), `Arc<Mutex>` state, call tracking via `GitProviderCall` enum. Clone helper for `GitProviderError` (non-Clone type).
- **Task 3:** `MockNotifier` with `calls()`, `story_calls()`, `summary_calls()` accessors.
- **Task 4:** `MockSessionRunner` with configurable `SessionOutcome`, `check_and_recover_wal()` returns `None`.
- **Task 5:** `MockReviewRunner` with configurable `ReviewOutcome`.
- **Task 6:** All 6 fixture builders: `make_test_config`, `make_test_secrets`, `make_test_story`, `write_sprint_status`, `write_wal_file`, `create_test_repo`.
- **Task 7:** 38 integration tests — all pass. Covers mock behavior, fixture validity, YAML roundtrips, git repo creation, and Send+Sync bounds.
...
- `tests/integration/test_mocks.rs` — NEW (26 mock self-verification tests)
...
- `_bmad-output/implementation-artifacts/7-1-integration-test-infrastructure-fixtures.md` — MODIFIED (tasks checked, dev record updated)