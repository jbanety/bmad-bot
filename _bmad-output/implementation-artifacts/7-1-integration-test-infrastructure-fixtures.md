...
- **Task 0 (BLOCKER):** Created `src/lib.rs` exposing all non-CLI modules (`auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Removed corresponding `mod` declarations from `main.rs`, updated all `crate::` imports in `src/cli/` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 934 existing tests pass.
- **Task 1:** Created `tests/integration.rs` entry point with `#[path]` attributes pointing to `tests/integration/` subdirectory. Created `tests/integration/helpers/{mod.rs, mocks.rs, fixtures.rs}`, `tests/integration/test_mocks.rs`, `tests/integration/test_fixtures.rs`.
- **Task 2:** Implemented `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), `Arc<Mutex<...>>` for configurable results, `calls()` for call tracking. Implements `GitProvider` trait, satisfies `Send + Sync`.
- **Task 3:** Implemented `MockNotifier` capturing all `notify_story`/`notify_run_summary` calls into `Arc<Mutex<Vec<NotifierCall>>>`. Provides `calls()`, `story_calls()`, `summary_calls()` accessors. Satisfies `Send + Sync`.
- **Task 4:** Implemented `MockSessionRunner` — standalone struct (no trait) returning configurable `SessionOutcome`. Includes `run()`, `check_and_recover_wal()` (always `None`), and call tracking.
- **Task 5:** Implemented `MockReviewRunner` — standalone struct returning configurable `ReviewOutcome`. Includes `run()` and `run_calls()` tracking.
- **Task 6:** Implemented all fixture builders: `make_test_config(dir)`, `make_test_secrets()`, `make_test_story(key, label, deps)`, `write_sprint_status(dir, entries)`, `write_wal_file(dir, state)`, `create_test_repo(dir)`. All produce valid data structures verified by roundtrip tests.
- **Task 7:** 32 self-verification integration tests covering all mocks and fixtures. Tests validate: return values, call tracking, `Send + Sync` bounds, YAML roundtrips, git repo HEAD existence, config validation.
- **Decision:** Included `auth` and `llm` modules in `lib.rs` (not listed in task 0.1) because they are transitively required by `session`, `review`, `pipeline`, and `supervisor` modules via `use crate::auth/llm` imports.
- **Decision:** Used `#[path = "integration/..."]` attributes in `tests/integration.rs` because Rust crate-root module resolution resolves submodules relative to `tests/` not `tests/integration/`, even in edition 2024.
- **Decision:** Added `git2 = "0.20"` to `[dev-dependencies]` since integration tests are separate crates and cannot access main `[dependencies]` directly.

### File List

- `src/lib.rs` — NEW: Library crate root exposing all non-CLI modules
- `src/main.rs` — MODIFIED: Removed `mod` declarations for non-CLI modules, kept `mod cli;`
- `src/session/mod.rs` — MODIFIED: Added `pub use state::{ChatMessage, SessionState};` re-export
- `src/cli/mod.rs` — MODIFIED: Changed `crate::` imports to `bmad_bot::` for config, auth, session, watcher, pipeline
- `src/cli/state.rs` — MODIFIED: Changed `crate::config::discovery` to `bmad_bot::config::discovery`
- `src/cli/git_detect.rs` — MODIFIED: Updated doc comment reference from `crate::` to `bmad_bot::`
- `Cargo.toml` — MODIFIED: Added `git2 = "0.20"` to `[dev-dependencies]`
- `tests/integration.rs` — NEW: Integration test entry point with `#[path]` module declarations
- `tests/integration/helpers/mod.rs` — NEW: Re-exports `mocks` and `fixtures` modules
...
- `tests/integration/test_mocks.rs` — NEW: 20 self-verification tests for all mock implementations
- `tests/integration/test_fixtures.rs` — NEW: 12 self-verification tests for all fixture builders
