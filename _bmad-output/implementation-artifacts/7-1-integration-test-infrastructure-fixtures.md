...
- **Task 0:** Created `src/lib.rs` exposing all non-CLI modules (`auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Removed all `mod X;` from `main.rs` except `mod cli;`. Updated all `crate::` references in `src/cli/` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 830 lib tests + 104 binary tests pass.
- **Task 1:** Created `tests/integration.rs` as Cargo test binary entry point using `#[path]` attributes (required by Rust 2024 edition). Created `tests/integration/helpers/mod.rs`, `mocks.rs`, `fixtures.rs`.
- **Task 2:** `MockGitProvider` with `Arc<Mutex<>>` fields, builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), call tracking via `Vec<GitProviderCall>`. Implements `GitProvider` trait. `Send + Sync` verified.
- **Task 3:** `MockNotifier` captures all calls. Builder pattern with error injection (`with_story_error`, `with_summary_error`). `calls()`, `story_calls()`, `summary_calls()` accessors. `Send + Sync` verified.
- **Task 4:** `MockSessionRunner` standalone struct with configurable outcome via closure (`with_outcome`). WAL recovery simulation (`with_wal_recovery`). Call tracking.
- **Task 5:** `MockReviewRunner` standalone struct with configurable outcome. Call tracking.
- **Task 6:** All 6 fixture builders: `make_test_config`, `make_test_secrets`, `make_test_story`, `write_sprint_status`, `write_wal_file`, `create_test_repo`. All produce valid, parseable outputs.
- **Task 7:** 35 self-verification tests covering all mocks and fixtures. All pass. `Send + Sync` bounds verified for all 4 mock types.
- **Decision:** `auth` and `llm` modules added to `lib.rs` (not in original task list) because they are dependencies of `session`, `review`, `supervisor`, and `pipeline` modules. Without them, compilation fails.
- **Decision:** Used `#[path = "integration/..."]` attributes in `tests/integration.rs` because Rust 2024 edition does not automatically resolve submodule paths for test binary entry points.
...
- src/cli/mod.rs (MODIFIED — `crate::` → `bmad_bot::` for all non-CLI module references)
...
- _bmad-output/implementation-artifacts/7-1-integration-test-infrastructure-fixtures.md (MODIFIED — tasks marked, dev record)