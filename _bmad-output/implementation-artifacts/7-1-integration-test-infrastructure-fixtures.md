...
- **Task 0**: Expanded `src/lib.rs` from 2 modules (config, mcp) to all 12 non-CLI modules (auth, config, git_provider, llm, mcp, notifier, pipeline, review, session, supervisor, tools, watcher). Updated `src/main.rs` to keep only `mod cli;`. Changed all `crate::` references in `src/cli/` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 886 lib + 104 bin tests pass.
- **Task 1**: Created `tests/integration.rs` entry point with `#[path]` attributes for submodule resolution (edition 2024 test binary module paths). Created `tests/integration/helpers/{mod.rs, mocks.rs, fixtures.rs}` and test files.
- **Tasks 2-5**: Implemented MockGitProvider (GitProvider trait), MockNotifier (Notifier trait), MockSessionRunner, MockReviewRunner. All use Arc<Mutex<>> for interior mutability, builder pattern for configuration, call-tracking Vec for assertions. All satisfy Send + Sync bounds.
- **Task 6**: Implemented all 6 fixture builders: make_test_config, make_test_secrets, make_test_story, write_sprint_status, write_wal_file, create_test_repo. All use actual codebase types and produce valid data structures.
- **Task 7**: 28 self-verification tests covering all mocks and fixtures. Tests verify: configured return values, call tracking, notification capture, YAML parseability, git repo creation, Send+Sync bounds.
- **Decision**: Included all 12 non-CLI modules in lib.rs (not just the 9 in dev notes) because auth, llm, and mcp are referenced via `crate::` across many modules — omitting them would break compilation.
- **Decision**: Used `#[path]` attributes in `tests/integration.rs` because Rust 2024 edition still resolves test binary submodules relative to `tests/` not the file's name directory.

### File List
- `src/lib.rs` — MODIFIED (expanded from 2 to 12 pub mod declarations)
- `src/main.rs` — MODIFIED (removed 12 mod declarations, kept only mod cli)
- `src/session/mod.rs` — MODIFIED (added pub use state::{SessionState, ChatMessage})
- `src/cli/mod.rs` — MODIFIED (changed crate:: to bmad_bot:: for 16 references)
- `src/cli/state.rs` — MODIFIED (changed crate:: to bmad_bot:: for 2 references)
...
- `tests/integration/test_mocks.rs` — NEW (20 mock self-verification tests)
