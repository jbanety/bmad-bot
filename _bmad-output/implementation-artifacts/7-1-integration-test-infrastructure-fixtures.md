...
- Task 0: Expanded `src/lib.rs` from 2 modules (config, mcp) to 12 modules (all except cli). Updated `src/main.rs` to only keep `mod cli;`. Updated all `crate::` references in cli to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` to `src/session/mod.rs`. Eliminated duplicate compilation — test count went from 1064 (74 lib + 990 bin with overlap) to 990 unique tests (886 lib + 104 bin).
- Task 0 decision: exposed ALL modules (auth, llm, mcp, supervisor, tools) in lib.rs, not just those listed in the story, because cli module references them via `bmad_bot::` after the refactor and other modules have cross-dependencies.
- Tasks 1-6: Created full integration test directory structure with `#[path]` attributes in `tests/integration.rs` (required by Rust 2024 edition which does not auto-resolve `tests/integration/` as submodule dir).
