...
- [x] Task 1: Create `tests/integration/` directory structure (AC: #1, #3)
  - [x] 1.1 Create `tests/integration/main.rs` as the Cargo test binary entry point
...
### Agent Model Used
Claude claude-sonnet-4-20250514 (via Anthropic)

### Debug Log References
- Pre-existing test failure: `session::cleanup::tests::test_unblock_dependents_no_partial_key_match` — confirmed failing before this story (git stash verified). Not introduced by this story.

### Completion Notes List
- Task 0: Created `src/lib.rs` exposing all modules (auth, config, git_provider, llm, notifier, pipeline, review, session, supervisor, tools, watcher). Updated `src/main.rs` to keep only `mod cli;`. Updated `src/cli/` imports from `crate::` to `bmad_bot::` since cli is now in the binary crate while other modules are in the library crate. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`.
- Task 1: Used `tests/integration/main.rs` (not `tests/integration.rs`) as entry point — Cargo edition 2024 discovers `tests/{name}/main.rs` correctly. The `tests/integration.rs` + `tests/integration/` directory pattern from the story spec doesn't work because `tests/integration.rs` is a crate root and `mod helpers;` looks for `tests/helpers.rs` not `tests/integration/helpers.rs`.
- Tasks 2-5: All mocks implemented with builder pattern + Arc<Mutex<...>> for Send+Sync. MockSessionRunner/MockReviewRunner are standalone structs (not trait impls) matching codebase pattern.
- Task 6: All fixture builders implemented. `BotSecrets` uses `github_copilot_oauth_token` (actual field name, not `github_models_api_key` as in story spec). Added `git2 = "0.20"` as dev-dependency (story incorrectly stated it was already present).
- Task 7: 32 integration tests covering all mocks, fixtures, and Send+Sync bounds.

### File List
- `src/lib.rs` — NEW: library crate root exposing all modules
- `src/main.rs` — MODIFIED: removed mod declarations except `mod cli;`
- `src/session/mod.rs` — MODIFIED: added `pub use state::{SessionState, ChatMessage};`
- `src/cli/mod.rs` — MODIFIED: `crate::` → `bmad_bot::` for auth, config, session, watcher, pipeline imports
- `src/cli/state.rs` — MODIFIED: `crate::` → `bmad_bot::` for config::discovery import
- `src/cli/git_detect.rs` — MODIFIED: doc comment `crate::` → `bmad_bot::`
- `Cargo.toml` — MODIFIED: added `git2 = "0.20"` to dev-dependencies
- `tests/integration/main.rs` — NEW: integration test binary entry point
...
- `tests/integration/test_fixtures.rs` — NEW: 11 tests for fixture builders
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — MODIFIED: 7-1 status → in-progress