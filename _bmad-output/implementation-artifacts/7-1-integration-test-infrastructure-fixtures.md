...
- **Task 0:** Created `src/lib.rs` exposing all modules (`auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Updated `src/main.rs` to only declare `mod cli;` — all other modules now accessed via `bmad_bot::*`. Updated all `crate::` references in `src/cli/` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. Build passes, all 837 existing unit tests pass.
- **Task 1:** Created `tests/integration.rs` entry point with `#[path]` attributes for Rust 2024 edition compatibility, plus `tests/integration/helpers/{mod.rs, mocks.rs, fixtures.rs}` and test files.
- **Task 2:** Implemented `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), call tracking via `Arc<Mutex<Vec<GitProviderCall>>>`, and `GitProvider` trait implementation. Includes `clone_git_provider_error` helper since `GitProviderError` is not `Clone`.
- **Task 3:** Implemented `MockNotifier` with call tracking, `story_calls()` / `summary_calls()` accessors, and error injection via `with_story_error` / `with_summary_error`.
- **Task 4:** Implemented `MockSessionRunner` as standalone struct (no shared trait) with configurable `SessionOutcome` and `check_and_recover_wal()` returning `None`.
- **Task 5:** Implemented `MockReviewRunner` as standalone struct with configurable `ReviewOutcome` and call tracking.
- **Task 6:** Implemented all 6 fixture builders: `make_test_config`, `make_test_secrets`, `make_test_story`, `write_sprint_status`, `write_wal_file`, `create_test_repo`. All use the exact types from the codebase.
- **Task 7:** Wrote 35 self-verification tests covering all mocks, fixtures, YAML parsing, git repo creation, and Send+Sync bounds. All pass.

### File List
- `src/lib.rs` — NEW: library crate exposing all modules for integration tests
...
- `src/cli/mod.rs` — MODIFIED: changed `crate::` to `bmad_bot::` for all library module references
- `src/cli/state.rs` — MODIFIED: changed `crate::config` to `bmad_bot::config`
- `src/cli/git_detect.rs` — MODIFIED: updated doc comment path reference
...
- `tests/integration/test_mocks.rs` — NEW: 21 self-verification tests for mocks
- `tests/integration/test_fixtures.rs` — NEW: 14 self-verification tests for fixtures
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — MODIFIED: story status updated
- `_bmad-output/implementation-artifacts/7-1-integration-test-infrastructure-fixtures.md` — MODIFIED: task checkboxes, status, dev agent record