...
- [x] Task 6: Implement fixture builder functions (AC: #2)
  ...
  - [x] 6.6 `create_test_repo(dir)` — initializes git repo with initial commit via Git CLI
...
- **Task 0:** Created `src/lib.rs` exposing all non-CLI modules (`auth`, `config`, `git_provider`, `llm_context`, `llm_logging`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`). Updated `src/main.rs` to keep only `mod cli;`. Changed `session::state` from `pub(crate)` to `pub`. Updated all `crate::` references in `cli/` to `bmad_bot::` since CLI is now in the binary crate. All 847 existing tests pass.
- **Task 1:** Created integration test directory structure: `tests/integration.rs` (entry point with `#[path]` attributes for Rust 2024 edition module resolution), `tests/integration/helpers/mod.rs`, `tests/integration/helpers/mocks.rs`, `tests/integration/helpers/fixtures.rs`, `tests/integration/test_mocks.rs`, `tests/integration/test_fixtures.rs`.
- **Task 2:** Implemented `MockGitProvider` with builder pattern (`with_create_pr`, `with_add_comment`, `with_get_pr_url`), `Arc<Mutex<...>>` for return values, call tracking via `GitProviderCall` enum, full `GitProvider` trait impl. Manual `clone_git_provider_error()` helper since `thiserror` types don't derive Clone.
- **Task 3:** Implemented `MockNotifier` with `NotifierCall` enum for captured calls, `calls()`, `story_calls()`, `summary_calls()` accessors, full `Notifier` trait impl.
- **Task 4:** Implemented `MockSessionRunner` as standalone struct (no shared trait with real runner per Dev Notes). Returns configurable `SessionOutcome` with manual clone helper. `check_and_recover_wal()` returns None. Call tracking via `SessionRunCall`.
- **Task 5:** Implemented `MockReviewRunner` matching `ReviewRunner::run` signature. Returns configurable `ReviewOutcome`. Call tracking via `ReviewRunCall`.
- **Task 6:** Implemented all 6 fixture builders. `make_test_config(dir)` produces valid `BotConfig` (polling=60, github, review=enabled). `make_test_secrets()` with DO-NOT-USE markers. `make_test_story(key, label, deps)` parses key for epic/story nums. `write_sprint_status(dir, entries)` writes all entry types under `development_status:`. `write_wal_file(dir, state)` serializes via serde_yml. `create_test_repo(dir)` uses Git CLI (not git2 which was removed in migration) with `--allow-empty` initial commit.
- **Task 7:** 36 self-verification tests: MockGitProvider (5 tests), MockNotifier (3 tests), MockSessionRunner (4 tests), MockReviewRunner (4 tests), fixtures (11 tests), Send+Sync (4 tests), write_sprint_status (3 tests), write_wal_file (2 tests), create_test_repo (2 tests).
- **Key decision:** `git2` crate was removed from the project per git-cli-migration. `create_test_repo()` uses `std::process::Command` with Git CLI instead of `git2::Repository::init`. Story Dev Notes referenced `git2` but project-context.md confirms removal.
- **Key decision:** `src/lib.rs` includes ALL non-CLI modules (not just the 9 listed in Task 0.1) because `review`, `session`, and `supervisor` depend on `auth`, `llm_context`, and `llm_logging`.
- **Key decision:** Used `#[path = "integration/..."]` attributes in `tests/integration.rs` because Rust 2024 edition crate-root test binaries resolve submodules relative to the `tests/` directory, not the file's name-derived subdirectory.

### File List
- `src/lib.rs` — NEW: library crate root exposing all non-CLI modules
- `src/main.rs` — MODIFIED: removed `mod` declarations (except `cli`), binary-only
- `src/session/mod.rs` — MODIFIED: `pub(crate) mod state` → `pub mod state`
- `src/cli/mod.rs` — MODIFIED: `crate::` → `bmad_bot::` for library module references
- `src/cli/state.rs` — MODIFIED: `crate::config` → `bmad_bot::config`
- `src/cli/git_detect.rs` — MODIFIED: doc comment path updated
...
- `tests/integration/test_mocks.rs` — NEW: 22 mock self-verification tests
- `tests/integration/test_fixtures.rs` — NEW: 14 fixture self-verification tests
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — MODIFIED: story 7-1 status updated

## Change Log
- Story 7.1 implemented: Integration test infrastructure with mocks (MockGitProvider, MockNotifier, MockSessionRunner, MockReviewRunner), fixture builders (make_test_config, make_test_secrets, make_test_story, write_sprint_status, write_wal_file, create_test_repo), and 36 self-verification tests. Created src/lib.rs to enable integration test imports. (Date: 2026-02-11)