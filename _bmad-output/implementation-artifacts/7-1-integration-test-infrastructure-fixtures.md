...
- **Task 0:** Created `src/lib.rs` with pub mod for all 11 modules (auth, config, git_provider, llm, notifier, pipeline, review, session, supervisor, tools, watcher). Updated `src/main.rs` to keep only `mod cli;`. Updated all `crate::` references in `src/cli/mod.rs`, `src/cli/state.rs` to `bmad_bot::`. Added `pub use state::{SessionState, ChatMessage};` re-export in `src/session/mod.rs`. All 849 lib + 104 bin tests pass.
- **Task 1:** Created `tests/integration.rs` as entry point with `#[path]` attributes pointing to `tests/integration/` submodules. Created `tests/integration/helpers/mod.rs`, `mocks.rs`, `fixtures.rs`.
- **Task 2:** `MockGitProvider` with `Arc<Mutex<...>>` builder pattern, full `GitProvider` trait impl, `GitProviderCall` enum for call tracking. `clone_git_provider_error` reconstructs all 11 variants.
- **Task 3:** `MockNotifier` captures all calls to `notify_story` / `notify_run_summary` via `NotifierCall` enum. Provides `calls()`, `story_calls()`, `summary_calls()` accessors.
- **Task 4:** `MockSessionRunner` standalone struct with configurable `SessionOutcome`, `SessionRunnerCall` tracking, `check_and_recover_wal` returns None.
- **Task 5:** `MockReviewRunner` standalone struct with configurable `ReviewOutcome`, `ReviewRunnerCall` tracking.
...
- **Task 7:** 33 integration tests covering all mocks (default returns, configured returns, call tracking, error returns), all fixtures (valid defaults, path correctness, validation, YAML parseability, git repo creation), and Send+Sync bounds.
