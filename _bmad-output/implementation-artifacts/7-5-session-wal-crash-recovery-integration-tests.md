...
- [x] Task 2: Create WAL fixture helpers (AC: #1, #2, #3, #4)
  ...
  - [x] 2.3 Add `fn make_wal_test_config(dir: &Path) -> Arc<BotConfig>` — reuses shared `make_test_config` then overrides `implementation_artifacts` to point directly at temp dir
  - [x] 2.4 Add `fn make_session_runner(config: Arc<BotConfig>) -> SessionRunner` — builds with real AgentFactory, empty McpManager (no LLM calls)
...
- [x] Task 3: Write full save→recover→parse integration test (AC: #1)
  ...
  - [x] 3.2 Construct `SessionRunner::new(config, agent_factory, shutdown, mcp_manager)` with config pointing to temp dir
...
- [x] Task 7: Write post-recovery pipeline test (AC: #5)
  - [x] 7.1 **Prerequisite:** Story 7.4 DI refactor is complete. Made `process_recovered_session` `pub` in `src/pipeline.rs` (see Dev Notes)
  - [x] 7.2 Build `StoryPipeline::new_with_components()` via `PipelineTestBuilder` with MockDevRunner, MockCodeReviewer, MockGitProvider, MockNotifier
  - [x] 7.3 Construct `StoryInfo` + `SessionOutcome::Completed` for "1-2-cli", call `process_recovered_session(&story, outcome)`
  - [x] 7.4 Assert `PipelineResult` has `status: Completed`, `pr_url: Some(...)`, MockGitProvider received `create_pr`
  - [x] 7.5 Repeat with `SessionOutcome::Failed` → assert `status: Error`, PR still created with `[NEEDS REVIEW]` in title
  - [x] 7.6 Repeat with `SessionOutcome::Escalated` → assert `status: Blocked`, escalation PR created

- [x] Task 8: Write recovery-first priority test (AC: #6)
  - [x] 8.1 Write valid WAL to temp dir, construct SessionRunner, call `check_and_recover_wal()` → assert `Some` (WAL detected)
  - [x] 8.2 With no WAL file (pipeline via `new_with_components`), call `recover_and_process()` → assert `None` (daemon proceeds to polling)
  - [x] 8.3 Note: `recover_and_process()` via `new_with_components()` returns `None` (session_runner_for_recovery is None). Test 8.1 uses real `SessionRunner` directly
...
- [x] Task 11: Verify all tests pass (AC: all)
  - [x] 11.1 `cargo test --test integration` — all 96 integration tests pass (12 new session WAL tests)
  - [x] 11.2 `cargo test` — no regressions, 1012 unit tests pass (lib+bin), 96 integration tests pass
  - [x] 11.3 `cargo clippy` — zero warnings from new code (pre-existing clippy errors in lib crate unrelated to this story)
...
- All 6 ACs implemented and tested via 12 integration tests in `tests/integration/test_session_wal.rs`
- AC #1: `test_wal_recovery_valid_returns_recovery_info` — full save→recover→parse chain via public API
- AC #2: `test_wal_to_rig_messages_preserves_all_messages` + `test_wal_to_rig_messages_empty_history`
- AC #3: `test_wal_corrupt_deletes_file_returns_none` — corrupt WAL deleted, returns None
- AC #4: `test_wal_no_file_returns_none` — no WAL returns None immediately
- AC #5: Three pipeline tests via `PipelineTestBuilder` — Completed/Failed/Escalated outcomes
- AC #6: `test_wal_recover_and_process_with_wal` + `test_wal_recover_and_process_without_wal_returns_none`
- Supplementary: Legacy WAL backward compatibility + forward compatibility (unknown YAML fields)
- `SessionRunner::new()` signature differs from story spec (`config, agent_factory, shutdown, mcp_manager` vs `config, secrets`). Built `make_session_runner()` helper with real `AgentFactory` + empty `McpManager`.
- Reused shared fixture helpers from Story 7.1 (`make_test_config`, `make_test_secrets`, `impl_artifacts_dir`, `PipelineTestBuilder`)
- Made `process_recovered_session` `pub` in `src/pipeline.rs` for AC #5 integration test access (per Dev Notes guidance)
- All 1012 unit tests pass, all 96 integration tests pass, no regressions

### File List
- `tests/integration/test_session_wal.rs` — NEW: 12 integration tests for WAL crash recovery
- `tests/integration.rs` — MODIFIED: Added `#[path = "integration/test_session_wal.rs"] mod test_session_wal;`
- `src/pipeline.rs` — MODIFIED: Changed `process_recovered_session` from `async fn` (private) to `pub async fn` for integration test access
- `_bmad-output/implementation-artifacts/7-5-session-wal-crash-recovery-integration-tests.md` — MODIFIED: Story file updates