...
- 19 integration tests covering all 5 ACs plus supplementary SprintStatusFile and edge-case tests
- Used `artifacts_dir()` helper to bridge gap between `make_test_config()` (which nests under `_bmad-output/implementation-artifacts`) and `write_sprint_status()` (which writes to given dir)
- Cyclic dependency tested via manual `StoryInfo` construction with injected circular deps (bypassing `derive_dependencies()` which only produces linear chains)
- Negative cascade tests confirm `in-progress` and `review` statuses do NOT trigger cascade blocking
- All tests are sync (`#[test]`), use `tempfile::tempdir()` for filesystem isolation

### File List
- tests/integration/test_watcher.rs (NEW — 19 integration tests)
- tests/integration.rs (MODIFIED — added `mod test_watcher` declaration)
- _bmad-output/implementation-artifacts/7-3-watcher-dependency-resolution-story-selection-integration-tests.md (MODIFIED — status + tasks marked complete)