...
- [x] Task 6: Write escalation test (AC: #3)
  - [x] 6.1 MockDevRunner returns `Escalated` → assert Blocked, pr_url is Some (actual code creates escalation PR), error_detail contains "Escalated"
  - [x] 6.2 Assert MockGitProvider: `create_pr` called (actual code creates escalation PR)
...
- **Task 0 (DI Refactor):** Added `DevRunner` and `CodeReviewer` async traits to `src/pipeline.rs`. Implemented for `SessionRunner` and `ReviewRunner`. Changed `StoryPipeline` struct to use trait objects (`Box<dyn DevRunner>`, `Box<dyn CodeReviewer>`) plus `session_runner_for_recovery: Option<SessionRunner>`. Added `new_with_components()` constructor for test injection. Updated all 3 call sites: `process_story()` → `dev_runner.run_dev_session()`, two review sites → `code_reviewer.run_review()`, `recover_and_process()` → `session_runner_for_recovery.as_ref()?`. All 66 existing tests pass, zero regressions.
- **Tasks 1-2 (Mocks & Builder):** Added `MockDevRunner` (VecDeque-based, supports multi-call) and `MockCodeReviewer` (with `never_called()` guard) to `tests/integration/helpers/mocks.rs`. Added `Clone` impls for `MockGitProvider` and `MockNotifier` (Arc-sharing). Added assertion helpers: `captured_create_pr_params()`, `captured_add_comment_calls()`, `create_pr_call_count()`, `add_comment_call_count()`, `story_notification_count()`, `run_summary_count()`. Added `PipelineTestBuilder` and `create_test_repo_with_remote()` to fixtures.
- **Tasks 3-11 (Integration Tests):** 8 integration tests in `tests/integration/test_pipeline.rs` covering all 7 ACs plus batch processing. Tests use real git repos (bare remote + working dir) for `push_branch()` validation. AC #3 deviation documented: actual code creates escalation PRs (unlike story spec "NO PR"), tests verify actual behavior.

### File List

- `src/pipeline.rs` — MODIFIED (added DevRunner/CodeReviewer traits + impls, refactored StoryPipeline struct, added new_with_components())
- `tests/integration.rs` — MODIFIED (added `mod test_pipeline` declaration)
- `tests/integration/helpers/mocks.rs` — MODIFIED (added MockDevRunner, MockCodeReviewer, Clone impls, assertion helpers)
...
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — MODIFIED (7-4 status: ready-for-dev → review)