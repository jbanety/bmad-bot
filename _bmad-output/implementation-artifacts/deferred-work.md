...
- **No cross-check between preamble tool names and rig Tool `NAME` constants:** `build_sub_agent_preamble` hardcodes `edit_file, read_file, grep, find_path, list_directory, git, terminal`. If any registered tool's `NAME` drifts (e.g., `list_dir`), the LLM attempts a non-existent tool. Add a small integration test that walks the registered tools and asserts each appears in the preamble string.
