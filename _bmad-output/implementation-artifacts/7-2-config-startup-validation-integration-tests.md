...
- ✅ Task 1: Created `tests/integration/test_config.rs` with `#[path]` declaration in `tests/integration.rs`
- ✅ Task 2: `test_config_valid_roundtrip_succeeds` + `test_config_valid_roundtrip_secrets_validate` — serialize → load → validate full pipeline
- ✅ Task 3: 7 tests covering invalid configs: zero polling, unknown git provider, unknown LLM provider, empty project_root, invalid YAML syntax, nonexistent file, field-name-in-error assertion
- ✅ Task 4: 4 tests covering missing secrets: anthropic key, github token, telegram token (when enabled), env var name in error message
- ✅ Task 5: 3 tests covering BMAD discovery: full structure, no _bmad dir, partial _bmad without config.yaml
- ✅ Task 6: 1 test verifying `build_http_client()` returns without panic
- Total: 17 new integration tests, all passing. Full suite 869+ tests pass with 0 failures.

### File List
- `tests/integration.rs` (modified — added `#[path = "integration/test_config.rs"] mod test_config;`)
- `tests/integration/test_config.rs` (new — 17 integration tests for config/secrets/discovery/http)
