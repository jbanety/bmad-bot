...
- **Cargo test convention:** `tests/integration.rs` is the binary entry point using `#[path]` attributes for module resolution (Rust crate roots don't support companion directory module resolution). To add a new test module, add `#[path = "integration/test_config.rs"] mod test_config;` in `tests/integration.rs` — do NOT use bare `mod test_config;`.
...
- **`src/lib.rs` exists** — Created by 7.1 Task 0. Exposes: `auth`, `config`, `git_provider`, `llm`, `notifier`, `pipeline`, `review`, `session`, `supervisor`, `tools`, `watcher`. Does NOT expose `cli` (intentional — cli is binary-only).
...
- All 48 tests pass (19 config integration tests + 29 existing). Zero regressions.

### Completion Notes List
- Task 1: Created `tests/integration/test_config.rs`, added `#[path]` declaration in `tests/integration.rs`
- Task 2: `test_config_valid_roundtrip_succeeds` — load → validate → secrets validate pipeline
- Task 3: 7 invalid config tests covering zero polling, unknown git/llm providers, empty project_root, invalid YAML, nonexistent file, error message field names
- Task 4: 4 secrets tests covering missing anthropic key, missing github token, missing telegram token, error message env var names
- Task 5: 3 BMAD discovery tests covering full structure, no _bmad/, partial _bmad/
- Task 6: 2 HTTP client tests covering no-panic and type assertion
- All tests use `tempfile::tempdir()` for filesystem isolation
- No env var manipulation — BotSecrets constructed directly per Dev Notes guidance
- Used `matches!()` with field guards for all error variant assertions

### File List
- `tests/integration/test_config.rs` (NEW) — 19 integration tests for config pipeline
- `tests/integration.rs` (MODIFIED) — added `mod test_config` declaration
- `_bmad-output/implementation-artifacts/sprint-status.yaml` (MODIFIED) — status update

### Change Log
- Story 7.2 implemented: 19 integration tests for config → startup validation pipeline (2026-02-11)