...
- [x] Task 2: Implement `ensure_story_branch` integration tests (AC: #1, #2)
  ...
  - [x] 2.5 Test: call on non-git directory → verify error returned (actual: `BaseBranchNotFound` — `RepoOpenFailed` variant does not exist in enum)
...
**Story 7.1 is IMPLEMENTED.** Integration test infrastructure exists in `tests/integration/`. Key facts from implementation:

1. **`src/lib.rs` exists** with 11 `pub mod` declarations (auth, config, git_provider, llm, notifier, pipeline, review, session, supervisor, tools, watcher). No Task 0 blocker needed.

2. **New test modules MUST use `#[path]` attributes** in `tests/integration.rs` (edition 2024 does not auto-resolve submodule paths for integration test crate roots):
   ```rust
   #[path = "integration/test_branch_git.rs"]
   mod test_branch_git;
   ```

3. **Test file naming convention:** `test_{module_name}.rs` (e.g., `test_config.rs`, `test_watcher.rs`, `test_notifier.rs`). For this story: `test_branch_git.rs`.
...
5. **Story 7.6 (Git Provider) is IMPLEMENTED** — tests factory construction, pure functions (`build_pr_description`, `build_pr_title`), and local `get_pr_url()`. No mock HTTP was used (no real API calls). Different from this story which tests local git operations via Git CLI subprocess. No overlap.
...
None — all tests passed on first run (except one minor fix: `src/main.rs` path in test required creating parent dir; switched to flat file path).

### Completion Notes List

- **Task 0**: `src/lib.rs` already exists from Story 7.1 — no changes needed. `cargo build` + `cargo test` verified (119 existing tests pass).
- **Task 1**: `tests/integration.rs` and `tests/integration/helpers/` already exist from Story 7.1. Created `tests/integration/test_branch_git.rs` (22 tests) and declared `#[path]` module in `tests/integration.rs`.
- **Task 2**: 5 tests for `ensure_story_branch` — Created/Reused variants, non-main parent, BaseBranchNotFound, non-git directory error.
- **Task 2.5 deviation**: Story specified `BranchError::RepoOpenFailed` but this variant doesn't exist in the enum. Actual behavior returns `BaseBranchNotFound` (since `branch_exists` returns false on non-git dirs). Test asserts `is_err()` which covers the intent.
- **Task 3**: 4 tests for `determine_base_branch` — no deps, dep exists, dep missing, multiple deps (last used).
- **Task 4**: 3 end-to-end tests — chained branch from dependency, multi-story chain (1-1→1-2→1-3), missing dep fallback to main.
- **Task 5**: 5 GitTool tests — branch_create+checkout, add+commit+log, status dirty/clean, diff, full roundtrip with SHA verification.
- **Task 6**: 3 preserve_partial_work tests — dirty tree WIP commit, clean tree no-op, WIP on story branch.
- **Task 7**: 2 cross-module tests — full lifecycle (StoryInfo→determine→ensure→GitTool→preserve), branch switching consistency (file presence/absence across branches).
- **Total**: 22 new integration tests, all passing. Full suite: 141 tests, 0 failures.
- Reused existing helpers: `create_test_repo`, `make_test_story` from `tests/integration/helpers/fixtures.rs` (Story 7.1).

### Change Log

- Story 7.8 implemented: 22 integration tests for branch management, GitTool, and preserve_partial_work (Date: 2026-02-11)

### File List

- `tests/integration/test_branch_git.rs` — NEW (22 integration tests)
- `tests/integration.rs` — MODIFIED (added `test_branch_git` module declaration)
