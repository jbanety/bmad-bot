...
**Story 7.1 — ✅ IMPLEMENTED.** Key facts:

1. **`lib.rs` blocker is RESOLVED.** `src/lib.rs` exists with 11 modules: auth, config, git_provider, llm, notifier, pipeline, review, session, supervisor, tools, watcher. **`cli` is NOT in lib.rs** — it stays as `mod cli;` in `main.rs` (binary-only). Integration tests cannot import `bmad_bot::cli::*`.

2. **⚠️ Module declaration pattern:** `tests/integration.rs` uses `#[path]` attributes, NOT plain `mod` declarations. To add: `#[path = "integration/test_branch_git.rs"] mod test_branch_git;`

3. **Mock infrastructure exists:** `MockGitProvider`, `MockNotifier`, `MockSessionRunner`, `MockReviewRunner` in `tests/integration/helpers/mocks.rs`. These are NOT needed for Story 7.8 (branch management uses real Git CLI on temp repos).

4. **Fixture builders exist:** `create_test_repo(dir)`, `make_test_story(key, label, deps)`, `make_test_config(dir)` in `tests/integration/helpers/fixtures.rs`. No need to create inline helpers.
