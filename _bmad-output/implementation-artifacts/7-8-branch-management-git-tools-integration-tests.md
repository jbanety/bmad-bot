...
- [x] Task 2: Implement `ensure_story_branch` integration tests (AC: #1, #2)
  ...
  - [x] 2.5 Test: call on non-git directory → verify `BranchError::BaseBranchNotFound` (API has no `RepoOpenFailed` variant; `branch_exists()` returns false on non-repo, triggering `BaseBranchNotFound`)
...
- **Task 0**: `src/lib.rs` already existed (Story 7.1 implemented). Verified `cargo build` + `cargo test` pass (121 existing tests).
- **Task 1**: Created `tests/integration/test_branch_git.rs` and added `#[path]` declaration in `tests/integration.rs`. Reused existing `helpers/fixtures.rs` (`create_test_repo`, `make_test_story`).
- **Task 2**: 5 tests for `ensure_story_branch` — Created, Reused, from-parent-branch, BaseBranchNotFound, non-git-directory. Note: `BranchError::RepoOpenFailed` does not exist; non-git directory returns `BaseBranchNotFound` because `branch_exists()` returns `false` on CLI failure.
- **Task 3**: 4 tests for `determine_base_branch` — no deps, dep exists, dep missing (fallback), multiple deps (uses last).
- **Task 4**: 3 E2E flow tests — chained from dependency, multi-story chain (1-1 → 1-2 → 1-3 with SHA verification), dependency missing fallback.
- **Task 5**: 5 GitTool tests — branch_create+checkout, add+commit+log, status dirty/clean, diff, full roundtrip with SHA verification. All local actions only (no push/clone).
...
- 2025-01-XX: Implemented Story 7.8 — 22 integration tests for branch management, GitTool, and preserve_partial_work across 8 tasks.
