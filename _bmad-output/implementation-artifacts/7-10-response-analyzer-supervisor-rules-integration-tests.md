...
1. **`lib.rs` blocker is RESOLVED** — `src/lib.rs` now exposes all 12 modules (auth, config, git_provider, llm, mcp, notifier, pipeline, review, session, supervisor, tools, watcher). No Task 0 needed in downstream stories.

2. **Module declaration pattern:** `tests/integration.rs` uses `#[path = "integration/test_response_analyzer_supervisor.rs"] mod test_response_analyzer_supervisor;` (NOT plain `mod`). Cargo does NOT auto-resolve submodule directories for test binaries.

3. **Import pattern:** `use super::helpers::fixtures::...;` and `use super::helpers::mocks::...;` (NOT `crate::helpers` — use `super::` because test modules are siblings of `helpers`).
