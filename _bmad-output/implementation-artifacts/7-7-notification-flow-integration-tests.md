...
- **Task 0:** `src/lib.rs` already exists (Story 7.1 implemented). 12 pub modules including `notifier`. Verified `cargo build` + `cargo test` pass.
- **Task 1:** Created `tests/integration/test_notifier.rs`, registered via `#[path]` attribute in `tests/integration.rs`. Imports: `bmad_bot::notifier::*`, `bmad_bot::config::*`, `MockNotifier` from helpers.
- **Task 2:** 3 tests — `TelegramNotifier::new()` success/disabled, `StoryNotification` struct construction from external crate.
- **Task 3:** 2 tests — `create_notifier()` disabled path returns NoopNotifier behavior for both `notify_story` and `notify_run_summary`.
- **Task 4:** 2 tests — `create_notifier()` graceful fallback: `None` token and empty string token both return NoopNotifier.
- **Task 5:** 1 test — `create_notifier()` with valid token returns TelegramNotifier (confirmed by HTTP error on `notify_story()` call, not `Ok(())`).
- **Task 6:** 3 tests — `RunSummary` manual construction with correct counts, MockNotifier capture for `notify_run_summary` (1 call, correct fields), 3 `notify_story` calls captured with correct data.
