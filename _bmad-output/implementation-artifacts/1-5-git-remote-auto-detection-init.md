...
- [ ] Task 1: Implement URL parsing for SSH and HTTPS formats (AC: #5, #6)
  ...
  - [ ] 1.2 Handle SSH format: `git@<host>:<owner>/<repo>.git` — split on `:`, then split path on `/`
  - [ ] 1.3 Handle HTTPS format: `https://<host>/<owner>/<repo>.git` — standard URL path parsing
...
- [ ] Task 2: Integrate auto-detection into `collect_config_interactively()` (AC: #1, #2, #3, #4, #8, #10)
  ...
  - [ ] 2.3 If detection succeeds → display a summary block:
        ...
        ```
  - [ ] 2.4 If provider is `None` (unrecognized host) → display partial summary (owner + repo + branch) with a note: `"⚠ Git host not recognized — provider must be selected manually"`
  - [ ] 2.5 If user confirms → use detected values, skip manual git prompts entirely
...
- [ ] Task 3: Write unit tests (AC: #1–#10)
  ...
  - [ ] 3.4 Test `parse_git_remote_url` with GitLab URL → `("gitlab.com", "owner", "repo")`
  - [ ] 3.5 Test `parse_git_remote_url` with self-hosted URL → `("git.company.com", "owner", "repo")`
  - [ ] 3.6 Test `parse_git_remote_url` strips `.git` suffix correctly
  - [ ] 3.7 Test `parse_git_remote_url` works without `.git` suffix
  - [ ] 3.8 Test `parse_git_remote_url` with trailing slash
  - [ ] 3.9 Test `parse_git_remote_url` with port number in SSH URL
  - [ ] 3.10 Test `parse_git_remote_url` returns `None` for malformed URLs
...
- [ ] Task 4: Final quality checks
  ...
  - [ ] 4.2 Run `cargo clippy` and fix any warnings
  - [ ] 4.3 Run `cargo test` and verify all tests pass (including all Story 1.1–1.4 tests — no regressions)
  - [ ] 4.4 Verify all new public items have `///` doc comments
