...
//!
//! Each function builds a complete, valid data structure with sensible defaults.
...
pub fn make_test_config(dir: &Path) -> BotConfig {
    ...
    BotConfig {
        ...
        bmad_paths: BmadPathsConfig {
            ...
            implementation_artifacts: format!(
                "{dir_str}/_bmad-output/implementation-artifacts"
            ),
        },
...
/// Build `BotSecrets` with dummy tokens. NEVER use real keys.
...
///
/// Key format: `"{epic}-{story}-{slug}"` (e.g., `"7-1-integration-test-infrastructure"`).
...
pub fn write_wal_file(dir: &Path, state: &SessionState) {
    let yaml =
        serde_yml::to_string(state).expect("Failed to serialize SessionState to YAML");
...
///
/// Uses Git CLI (git2 was removed in Story 4.4).
